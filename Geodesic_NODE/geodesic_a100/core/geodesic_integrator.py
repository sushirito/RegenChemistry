...
Optimized for A100 parallel processing of 18,030 simultaneous geodesics
"""
    ...
    def integrate_batch(self,
        ...
        if self.use_adjoint and initial_states.requires_grad:
            ...
            trajectories = odeint_adjoint(
                ...
                t_span,
                method=method,
                rtol=torch.tensor(rtol, dtype=torch.float32, device=initial_states.device),
                atol=torch.tensor(atol, dtype=torch.float32, device=initial_states.device)
            )
        else:
